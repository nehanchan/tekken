...
// シンプルな削除関数（初回のみ実行）
const removeResendButtons = () => {
  const buttons = document.querySelectorAll('button[type="button"]');
  buttons.forEach((button: Element) => {
    const htmlButton = button as HTMLElement;
    const text = htmlButton.textContent || '';
    if (text.includes('再送信') || text.includes('Resend') || text.trim() === '') {
...
export default function MemoLayout({
  ...
  useEffect(() => {
    // 初回のみ3回実行（段階的）
    setTimeout(removeResendButtons, 500);
    setTimeout(removeResendButtons, 1000);
    setTimeout(removeResendButtons, 2000);
  ...
  return (
    <>
      ...
      <style jsx global>{`
        ...
        /* 2番目以降のボタンを非表示 */
        [data-amplify-router="confirmSignUp"] button:nth-of-type(n+2):not([type="submit"]),
        [data-amplify-router="confirmResetPassword"] button:nth-of-type(n+2):not([type="submit"]) {
          display: none !important;
        }
      `}</style>
